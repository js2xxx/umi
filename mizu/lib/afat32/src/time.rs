...
impl Time {
    ...
    /// * `millis` - number of milliseconds after the second in the range [0,
    ///   999]
    ///
    /// # Panics
    ///
...
///
/// Provides a custom implementation for a time resolution used when updating
/// directory entry time fields. `TimeProvider` is specified by the
/// `time_provider` property in `FsOptions` struct.
pub trait TimeProvider: Debug + Send + Sync + 'static {
    fn get_current_date(&self) -> Date;
    fn get_current_date_time(&self) -> DateTime;
}

/// `TimeProvider` implementation that always returns DOS minimal date-time
/// (1980-01-01 00:00:00).
