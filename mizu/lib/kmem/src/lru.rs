//! Copied from [`lru`](https://github.com/jeromefroe/lru-rs).
...
impl<K: Hash + Eq, V> LruCache<K, V> {
    ...
    pub fn new(cap: NonZeroUsize) -> LruCache<K, V> {
        LruCache::construct(
            cap,
            HashMap::with_capacity_and_hasher(cap.get(), RandomState::new()),
        )
    }
...
impl<K: Hash + Eq, V, S: BuildHasher> LruCache<K, V, S> {
    ...
    /// exists in the cache or another cache entry is removed (due to the
    /// lru's capacity), then it returns the old entry's key-value pair.
    ...
    // node if space is available. Shared between `put`, `push`,
    // `get_or_insert`, and `get_or_insert_mut`.
    ...
    /// is not present in the cache. Moves the key to the head of the LRU
    /// list if it exists.
    ///
    /// # Example
    ///
    ...
    /// Returns a mutable reference to the value of the key in the cache if it
    /// is present in the cache and moves the key to the head of the LRU
    /// list. If the key does not exist the provided `FnOnce` is used to
    /// populate the list and a mutable reference is returned.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// `peek_mut` does not update the LRU list so the key's position will
    /// be unchanged.
    ///
    /// # Example
    ///
    ...
    /// Returns the value corresponding to the least recently used item or
    /// `None` if the cache is empty. Like `peek`, `peek_lru` does not
    /// update the LRU list so the item's position will be unchanged.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Removes and returns the key and value corresponding to the least
    /// recently used item or `None` if the cache is empty.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Resizes the cache. If the new capacity is smaller than the size of the
    /// current cache any entries past the new capacity are discarded.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// An iterator visiting all entries in most-recently used order. The
    ...
    /// mutable reference on V.  The iterator element type is `(&K, &mut
    /// V)`.
    ///
    /// # Examples
    ///
...
// The compiler does not automatically derive Send and Sync for LruCache because
// it contains raw pointers. The raw pointers are safely encapsulated by
// LruCache though so we can implement Send and Sync for it below.
...
///
/// This `struct` is created by the [`iter`] method on [`LruCache`][`LruCache`].
...
/// [`LruCache`][`LruCache`]. See its documentation for more.
///
