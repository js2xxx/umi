...
///
/// Copied from [`async-lock`](https://github.com/smol-rs/async-lock).
#[derive(Debug)]
...
impl Semaphore {
    ...
    ///
    /// If the permit could not be acquired at this time, then [`None`] is
...
impl Semaphore {
    ...
    /// See `try_acquire_arc` for more information.
    pub fn try_acquire_arsc(self: &Arsc<Self>) -> Option<SemaphoreGuardArsc> {
        let mut count = self.count.load(Ordering::Acquire);
        loop {
            if count == 0 {
                return None;
            ...
            match self.count.compare_exchange_weak(
                ...
                Ok(_) => return Some(SemaphoreGuardArsc(self.clone())),
                Err(c) => count = c,
            }
        }
    }
    ...
    /// See `acquire_arc` for more information.
    pub fn acquire_arsc(self: &Arsc<Self>) -> AcquireArsc {
        AcquireArsc {
            semaphore: self.clone(),
            listener: None,
        }
...
/// The future returned by [`Semaphore::acquire_arc`].
pub struct AcquireArsc {
    /// The semaphore being acquired.
    semaphore: Arsc<Semaphore>,

    /// The listener waiting on the semaphore.
    listener: Option<EventListener>,
}

impl fmt::Debug for AcquireArsc {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("AcquireArc { .. }")
    }
}

impl Unpin for AcquireArsc {}

impl Future for AcquireArsc {
    type Output = SemaphoreGuardArsc;

    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let this = self.get_mut();

        loop {
            match this.semaphore.try_acquire_arsc() {
                Some(guard) => {
                    this.listener = None;
                    return Poll::Ready(guard);
                }
...
#[clippy::has_significant_drop]
#[derive(Debug)]
pub struct SemaphoreGuardArsc(Arsc<Semaphore>);

impl Drop for SemaphoreGuardArsc {
    fn drop(&mut self) {
        self.0.count.fetch_add(1, Ordering::AcqRel);
        self.0.event.notify(1);
    }
